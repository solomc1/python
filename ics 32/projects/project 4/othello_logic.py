...
class Othello:
    ...
    def make_move(self, row: int, col: int)->None:
        ...
        '''
        if self.game_over():
            raise invalid_move_exception("Game Over")
...
class Board:
    ...
    def __init__(self, rows: int, cols: int, original_board: bool):
        if (4 <= rows <= 16) and (rows % 2 == 0):
            ...
            raise beginning_condition_exception("Rows must be an even number between 4 and 16, inclusive.")
        
        if (4 <= cols <= 16) and (cols % 2 == 0):
            self.cols = cols
        else:
            raise beginning_condition_exception("Columns must be an even number between 4 and 16, inclusive.")
    ...
    def make_move(self, p: Point, piece: Piece)->None:
        if not self.on_board(p):
            raise invalid_move_exception("That location is not on the board")
        if not Piece.is_playable_piece(piece):
            raise invalid_move_exception("Cannot place a NONE piece (How did you do that? Damn hackers :/ )")
        
        if self.valid_move(p, piece):
            self.set_piece(p, piece)
            self._flip_list(self._locs_to_flip)
        else:
            raise invalid_move_exception("That is not a valid move")
